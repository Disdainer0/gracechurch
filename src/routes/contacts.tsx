...
import { getContacts } from "@/lib/sanity/queries";
  ...
  head: () => ({
    meta: [
      { title: "Контакти та розклад богослужінь — Церква Благодать" },
      {
        name: "description",
        content:
          "Розклад богослужінь церкви «Благодать»: неділя об 11:00, середа і п'ятниця о 18:30 — молитовне служіння. м. Калуш, вул. Михайла Грушевського 25а.",
      },
...
function ContactsPage() {
  ...
  const pageTitle = contacts?.pageTitle || "РОЗКЛАД БОГОСЛУЖІНЬ";
  const sundayServiceTime = contacts?.sundayServiceTime || "Неділя об 11:00";
  const weekdayServiceDays = contacts?.weekdayServiceDays || "Середа і п'ятниця";
  const weekdayServiceTime = contacts?.weekdayServiceTime || "о 18:30";
  const weekdayServiceLabel = contacts?.weekdayServiceLabel || "Молитовне служіння";
  const address = contacts?.address || "м. Калуш, вул. Михайла Грушевського 25а";
  const googleMapsUrl = contacts?.googleMapsUrl || MAPS_URL;
  const mapEmbedUrl = contacts?.mapEmbedUrl || MAP_EMBED_URL;
      ...
      <section className="container-grace pt-4 pb-16 md:pb-24">
        ...
        <div className="mt-10 grid grid-cols-1 gap-10 md:grid-cols-12 md:gap-12 md:items-start">
          <div className="md:col-span-6 space-y-8">
            <p className="text-[22px] md:text-[32px] font-light">{sundayServiceTime}</p>

            <div className="text-[22px] md:text-[32px] font-light leading-tight">
              <p>
                {weekdayServiceDays} {weekdayServiceTime}
              </p>
              <p>{weekdayServiceLabel}</p>
            </div>
