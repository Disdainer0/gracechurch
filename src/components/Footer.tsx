...
const MAPS_URL =
  "https://www.google.com/maps/place/Tserkva+Khvye+%22Blahodat%CA%B9%22/@49.0215174,24.3695249,19.25z";

const fallbackSettings: Partial<SiteSettings> = {
  navigation: [
    { path: "/", label: "Головна" },
    { path: "/history", label: "Історія" },
    { path: "/ministry", label: "Служіння" },
...
export function Footer() {
  ...
  const data = settings ?? (fallbackSettings as SiteSettings);
      ...
      <div className="container-grace pt-8 pb-8">
        ...
        <div className="mt-8 flex md:justify-end">
          <nav className="flex flex-wrap md:justify-end gap-x-6 md:gap-x-10 gap-y-4 text-base md:text-[24px] uppercase">
            {data.navigation?.map((item) => (
              <Link key={item.path} to={item.path} className="transition-colors hover:text-accent">
                {item.label}
              </Link>
            ))}
          </nav>
        ...
        <div className="mt-12 space-y-6 text-sm md:text-[24px]">
          ...
          <div className="flex items-center gap-4 group">
            <svg
              ...
              strokeLinecap="round"
              strokeLinejoin="round"
              className="w-6 h-6 shrink-0 transition-colors group-hover:text-accent"
            >
              <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
              <polyline points="22,6 12,13 2,6" />
            </svg>

            <a href={`mailto:${data.email}`} className="transition-colors hover:text-accent">
              E-mail: {data.email}
            </a>
          </div>
        </div>
        ...
        <div className="mt-20 flex flex-col-reverse gap-8 md:flex-row md:items-center md:justify-between text-sm md:text-[20px]">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="leading-tight">{data.copyrightText}</span>
          </div>
        </div>
      </div>
    </footer>
