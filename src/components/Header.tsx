...
const fallbackLinks = {
  facebook: "https://www.facebook.com/blagodatkalush",
  youtube: "https://www.youtube.com/channel/UCxAgeSNE3xZbtzN8rqkMCTA",
  instagram: "https://www.instagram.com/grace_church_kalush?igshid=YmMyMTA2M2Y%3D",
};
...
export function Header({ overlay = false }: { overlay?: boolean }) {
  ...
  const nav = settings?.navigation ?? fallbackNav;
  const facebookUrl = settings?.facebookUrl ?? fallbackLinks.facebook;
  const youtubeUrl = settings?.youtubeUrl ?? fallbackLinks.youtube;
  const instagramUrl = settings?.instagramUrl ?? fallbackLinks.instagram;
  ...
  return (
    <header className={`${overlay ? "absolute" : "relative"} left-0 right-0 top-0 z-40 w-full`}>
      <div className="container-grace flex items-center justify-between py-5 md:py-7">
        ...
        >
          <span
            className={`w-full h-[2px] bg-current transition-all duration-300 ease-in-out ${open ? "rotate-45 translate-y-[7px]" : ""}`}
          ></span>
          <span
            className={`w-full h-[2px] bg-current transition-all duration-300 ease-in-out ${open ? "opacity-0" : ""}`}
          ></span>
          <span
            className={`w-full h-[2px] bg-current transition-all duration-300 ease-in-out ${open ? "-rotate-45 -translate-y-[7px]" : ""}`}
          ></span>
